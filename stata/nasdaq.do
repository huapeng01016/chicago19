...
tickers = Frame.connect('default')
detail = Frame.create('detail')
...
obs = tickers.getObsTotal()
detail.addObs(obs)

for i in range(obs):
	data = parse_finance_page(tickers.getAt(0, i))
	detail.store(None, i, data)
